    ...
    .controller('CommodityController', ['$scope', '$location', 'AuthService', 'CommodityService', 'UploadService', '$q',
        function($scope, $location, AuthService, CommodityService, UploadService, $q) {
            ...
            $scope.getCommodityCategory = function(category) {
                ...
                $scope.commodityCategory.commission_show = $scope.commodityCategory.commission;
                if ($scope.commodityCategory.commission == null) {
                    ...
                    $scope.commodityCategory.commission_show = $scope.commodityCategory.commission * 100;
                    $scope.commission = {
                        'isopen': false,
                        'text': '固定比例',
                        'unit': '%',
            ...
            $scope.addCommodityCategory = function() {
                $scope.commodityCategory = {
                    ...
                    'name': '',
                    'commission': null,
                    'commission_show': null,
                };
            }

            $scope.submitCommodityCategory = function() {
                if ($scope.commission.unit == '%') {
                    $scope.commodityCategory.commission = $scope.commodityCategory.commission_show / 100.0;
            ...
            $scope.$watchCollection('spe_type.state', function(newVal, oldVal) {
                if (newVal == 'yes' && oldVal == 'no') {
                    ...
                    var flag = 0;
                    if($scope.commodity.specifications && $scope.commodity.specifications.length > 1){
            ...
            $scope.getCommodity = function(id) {
                ...
                CommodityService.getCommodity(id).then(function(res) {
                    ...
                    $scope.commodity.commission_show = $scope.commodity.commission;
                    if ($scope.commodity.commission == null) {
                        $scope.commodity.commission_show = $scope.commodity.commission;
                        $scope.commodityCommission = {
                            'isopen': false,
                            'text': '无返还',
                    ...
                    }else if ($scope.commodity.commission >= 1) {
                        $scope.commodity.commission_show = $scope.commodity.commission;
                        $scope.commodityCommission = {
                            'isopen': false,
                            'text': '固定金额',
                        ...
                        $scope.commodity.commission_show = $scope.commodity.commission * 100;
                        $scope.commodityCommission = {
                            'isopen': false,
                            'text': '固定比例',
                            'unit': '%',
            ...
            $scope.submit = function(isValid) {
                if (isValid) {
                    ...
                    if ($scope.commodityCommission.unit == '%') {
                        $scope.commodity.commission = $scope.commodity.commission_show / 100.0;
                    }else if ($scope.commodityCommission.unit == '元') {
                        $scope.commodity.commission = $scope.commodity.commission_show;
                    }else if ($scope.commodityCommission.unit == '  ') {
                        $scope.commodity.commission = null;
                    }
                    if ($scope.commodity.id) {
                        CommodityService.updateCommodity($scope.commodity).then(function(res) {
                            ...
                            var index = _.findIndex($scope.commodityCategoryList, function(n){
                                return n.id = $scope.commodityCategory.id;
                            });
                            $scope.getCommodityCategory($scope.commodityCategoryList[index]);
                        }, function(res) {
                            alert(res.message);
