    ...
    */
    ...
    protected function getRecipientCard($cardIds,$recipient){
        ...
        elseif($recipient == 'REFERRER'){
            $referrer_cardIds = Card::whereIn('id',$cardIds)->lists('referrer_id');
            return Card::whereIn('id',$referrer_cardIds)->get();
        }
        elseif($recipient == 'ROOT_REFERRER'){
            $referrer_cardIds = Card::whereIn('id',$cardIds)->lists('referrer_id');
            $root_referrer_cardIds = Card::whereIn('id',$referrer_cardIds)->lists('referrer_id');
//            $referrer_cardIds=Card::whereIn('referrer_id',$cardIds)->select('id')->get();
            return Card::whereIn('id',$root_referrer_cardIds)->get();
        }
    }
...
//        $this->doJobs($cardIds,['type'=>'mass', 'reason'=>'裕达营销赠送'],$jobs);
        ...
        Card::whereIn('card_code',$param['data']['cards'])->increment('bonus', $param['data']['change_amount']);
        //记录日志
