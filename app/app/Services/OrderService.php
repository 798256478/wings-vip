...
//            return json_exception_response($e);
    ...
    // 参数说明
    // $totalFee 金额
    public function createQrCodeOrder($totalFee, $card)
    {
        try {
            //事务开始
            DB::beginTransaction();
            $result = $this->doCreateQrCodeOrder($totalFee, $card);
            DB::commit();
            return $result;
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function doCreateConsumeOrder($data, $card)
    {
        ...
        $order->bonus_require = isset($data['bonus_require']) ? $data['bonus_require'] : 0;
        $order->bonus_pay_amount = 0;
        $order->state = 'FINISH';
        $order->is_need_delivery = false;
    ...
    //线下扫描二维码订单
    private function doCreateQrCodeOrder($totalFee,$card)
    {
        $orderPayment = new OrderPayment();
        $orderPayment->type = 'WECHAT';
        $orderPayment->amount = $totalFee;
        $orderPayment->name = $this->getPaymentNameByType('WECHAT');

        $bonus_present = $this->calculateBonus([$orderPayment], 'CONSUME');
        ...
        $payConfig = $this->WechatPaymentPrepare($order, $card, $totalFee*100);
        return $payConfig;
    }
...
//            return $card;
    ...
    private function doOrderComplete($order)
    {
        ...
        //同步至裕达
        if(config('customer.' . user_domain() . '.sync') && $order->channel === 'WECHAT'){
...
//    同步两次失败后记录并通知管理员
    ...
    {
        $change_amount = $order->bonus_present - $order->bonus_pay_amount;
        foreach($order->orderPayments as $val){
            if($val->use_bonus > 0){
                $change_amount -= $val->use_bonus;
            }
        }
        $data = [
            'type'=>'ORDER',
...
//                TODO:记录订单同步失败
                ...
                $orderSynvFail->save();
//                TODO 通知管理员
                $smsServices = new SmsService();
                $smsServices->sendYudaAdmin();
    ...
    public function yudaOrderSyncToThis($param)
    {
        ...
        $data['bonus_present'] = isset($param['data']['bonus_present']) ? $param['data']['bonus_present'] : 0;
        $data['bonus_require'] = isset($param['data']['bonus_require']) ? $param['data']['bonus_require'] : 0;
