...
function passwordEncrypt($str)
{
    ...
    $str .= $tmp;
    $a = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256,config('customer.' . user_domain() . '.sync.mcrypt.key'),$str,
        MCRYPT_MODE_CBC,config('customer.' . user_domain() . '.sync.mcrypt.iv')));
//    dd($a);
    return $a;
}
