...
class Data_migrationController extends BaseController
{
    ...
    public function basic_table(){
        ...
        //商品
        //增加一个默认分类
        $categoryId = DB::connection('mysql')->table('commodity_categories')->insertGetId([
            'name' => '默认分类',
            'created_at' => date('Y-m-d H:i:s', time()),
            'updated_at' => date('Y-m-d H:i:s', time()),
        ]);
        ...
        if(count($order_consume) > 0){
            foreach ($order_consume as $consume) {
                $olduser = DB::connection('mysql2')->table('user')->where('user_id', $consume['operator']['id'])->first();
                if ($olduser == null) {
                    continue;
                }
