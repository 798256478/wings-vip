...
class ThemeService
{
    ...
    public function loadWechatCss($fileName)
    {
        ...
        if ($this->themeKey != 'default') {
            $path = public_path() . '\\common\\css\\wechat\\' . $themeKey . '\\' . $fileName . '.css';
            if (file_exists($path)){
                $loadString .= '<link href="/common/css/wechat/' . $themeKey . '/' . $fileName . '.css" rel="stylesheet">';
            }
        }
    ...
    public function getWechatImage($imageName)
    {
        ...
        if ($this->themeKey != 'default') {
            $path = public_path() . '\\common\\imgs\\wechat\\' . $themeKey . '\\' . $imageName;
            if (file_exists($path)){
                $url = '/common/imgs/wechat/' . $themeKey . '/' . $imageName;
            }
        }
