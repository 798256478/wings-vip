...
@section('content')
	<div class="container">
		...
		@if($experiment_data->progress_id==5||$experiment_data->progress_id==6)
			<div class="title"><h3>预约解读</h3></div>
			<form action="" id="userForm" method="post">
				<input type="hidden" name="experiment_data_id" value="{{$experiment_data->id}}">
...
//			数据验证
			...
			$("#userForm").submit(function(){
				...
				$.ajax({
					...
					success:function(data){
						alert('预约成功');
						window.location.href="/wechat/health/info/{{$experiment_data->id}}";
					}
				});
				return false;
			});
